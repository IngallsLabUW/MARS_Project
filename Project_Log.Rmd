...
Individual runs. Both standards mixes in water at series of concentrations.
  - Annotated by user-defined text library, total score, MS1 isotopes vs. msms spectrum
  - HILICNeg individual runs vs
  
HILICPosMS2.
  - Is there a matching HILICNegMS2?
  
HILIC Pos Batches
  - Mostly the same as the individual runs?
  - PeakID_HILC_Pos Missing standards looks like alignment results
  
AlignmentResults (HILICNeg, plain AlignmentResults)
  - What are the values (Lots of -2, various numbers from around 300 - 600) 
  
HILIC pos Batch1, Batch2? Same as HILICNeg_Individual and _Alignment?


Individual run #s at end are different collision energies
...
Peak ID should be used as the key to find the correct individual results. That should be the "key", and then the MSMS data is in the individual samples. 
-2 means the peak wasn't picked
...
- Integrate the standards' MS2 with that.  
